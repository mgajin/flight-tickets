        ...
        <v-toolbar dark flat>
          ...
          <v-toolbar-items class="hidden-xs-only">
            <v-btn
              ...
              v-text="item.title"
            ></v-btn>
            <v-btn text v-if="getToken" v-text="'logout'" @click="logout"></v-btn>
          </v-toolbar-items>
        </v-toolbar>
      </v-container>
    </v-app-bar>
...
export default {
  ...
  computed: {
    ...
    navItems: function () {
      ...
      if (this.getToken) {
        if (this.isAdmin) {
          items = [{ icon: "", title: "Admin", link: "/admin" }];
        } else {
          items = [{ icon: "", title: "Bookings", link: "/bookings" }];
        }
      } else {
